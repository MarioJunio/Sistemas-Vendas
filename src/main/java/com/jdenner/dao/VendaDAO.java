...
public class VendaDAO {
	...
	public static int quantidade(String filtro) throws Exception {
		Conexao c = new Conexao();
		String sql = "" + " SELECT COUNT(1) " + " FROM TBVENDA AS V" + " INNER JOIN TBCLIENTE AS C "
				+ " ON C.CODIGO = V.CODIGOCLIENTE " + " WHERE C.NOME LIKE ? ";
		PreparedStatement ps = c.getConexao().prepareStatement(sql);
	...
	public static ObservableList<Venda> listar(String filtro, int quantidade, int pagina) throws Exception {
		Conexao c = new Conexao();
		String sql = "" + " SELECT V.* " + " FROM TBVENDA AS V" + " INNER JOIN TBCLIENTE AS C "
				+ " ON C.CODIGO = V.CODIGOCLIENTE " + " WHERE C.NOME LIKE ? "
				+ " ORDER BY V.SITUACAO ASC, V.DATAVENDA DESC " + " LIMIT ?,? ";
		PreparedStatement ps = c.getConexao().prepareStatement(sql);
		ps.setString(1, "%" + filtro + "%");
